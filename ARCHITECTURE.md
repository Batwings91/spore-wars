...
- **Weapon visuals:** `BOLT[0..5]` caches original white/cyan procedural projectiles at startup. `drawGunMounts()` draws 1/2/3/5/6/6 mounts from the same `GUN` records used by firing. Enemy plasma uses its separate sprites/fallbacks. Legacy bolt PNGs remain packaged but are no longer drawn.
...
## Trader presentation
`shopScreen()` retains `trader_shop.webp` and its procedural merchant fallback. The right side is a four-column square grid sourced from `SHOP`: primary, defence, engine, ordnance, support and side weapon. Tiles expose focus and ready/need/equipped states with borders plus text, not colour alone. Arrow keys move spatially through the two rows; Down reaches Back when no tile exists below, Up returns to the remembered item, and pointer/touch hit the same `shopTileBounds()` rectangles. The lower detail panel always calls `drawEquipmentPreview()` for a complete ship with only the selected candidate slot overridden: saved launch fit from the title, actual in-run weapon/shield/Orb at a checkpoint. Buy and Back retain their existing rectangles.
...
Ordinary eligible air kills now drop cores 45% (30% original plus 15% from the empty 0.42..0.57 range); guaranteed weapons and bomb/shield/repair probabilities are unchanged. Each ground destruction adds one wreck and a core pickup with 50% probability (review: a guaranteed core roughly tripled first-sector income), guarded by destroyed for exactly-once rewards. Ground wrecks are cosmetic, anchored to worldScroll, capped 20, removed below screen and reset on new run/Continue/quit/checkpoint; boss suppression clears live units but retains wrecks until they scroll away.
...
Player presentation uses `drawShipAssembly()` for flight and Workshop previews. `EQUIPMENT`, `SHIP_MOUNTS`, `savedLoadout()`, `previewLoadout()` and `drawEquipmentModule()` are the source of truth for ownership, tiers, costs, firing profiles and rendering hooks. `player_hull.png` is a gated core sprite with `player.png`/`SHIP` fallbacks. Preview rendering receives explicit animation state and never mutates save or run state. Optional rocket and side-laser hardware expands the compact starter silhouette symmetrically; the side-laser fit also widens the playfield-edge clamp so installed emitters and their beam origins remain visible.
