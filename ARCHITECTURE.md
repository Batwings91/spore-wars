...
| `src/weapons.js` | Shared equipment catalogue and mounts, player bolts/guns, rocket ordnance and Seeker Orb. |
...
- **Save:** `localStorage['640k.sporewars.v3']` = `{cores,best,weapon,shield,engine,orb,rockets}`. `migrateSaveData()` preserves all existing values, defaults missing Orb ownership to 0, and grants Mk I rockets to an existing v3 record that lacks the new field. A genuinely fresh save starts without rockets.
...
Player presentation uses `drawShipAssembly()` for flight and Workshop previews. `EQUIPMENT`, `SHIP_MOUNTS`, `savedLoadout()`, `previewLoadout()` and `drawEquipmentModule()` are the source of truth for ownership, tiers, costs, firing profiles and rendering hooks. `player_hull.png` is a gated core sprite with `player.png`/`SHIP` fallbacks. Preview rendering receives explicit animation state and never mutates save or run state. The side-weapon slot exists at tier 0 with a no-op renderer and locked shop entry; no laser can be purchased, equipped or fired in Phase 1.
