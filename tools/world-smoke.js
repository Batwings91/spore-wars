...
(async()=>{
  ...
  const onMessage=async payload=>{const data=payload&&payload.data!==undefined?payload.data:payload,raw=typeof data==='string'?data:data&&typeof data.text==='function'?await data.text():Buffer.from(data).toString('utf8'),m=JSON.parse(raw);if(m.id&&pending[m.id]){clearTimeout(pending[m.id].timer);pending[m.id].resolve(m.result||m.error);delete pending[m.id];}
  ...
  if(typeof ws.on==='function')ws.on('message',onMessage);else ws.addEventListener('message',onMessage);
  ...
  await check("for(let s=1;s<=4;s++){const c=faunaDecor(s);if(c.width!==X(PW)||c.height!==FAUNA_DECOR_H||faunaDecor(s)!==c)throw Error('decor cache '+s);}if(FAUNA_DECOR.filter(Boolean).length!==1||!FAUNA_DECOR[4])throw Error('decor eviction');if(WALL_FAUNA_LIMIT!==3)throw Error('fauna cap');if(!IMG.wall_fauna||IMG.wall_fauna.width!==768||IMG.wall_fauna.height!==232)throw Error('painted fauna sheet');if([wallFaunaFrame('spitter',false),wallFaunaFrame('spitter',true),wallFaunaFrame('snap',false),wallFaunaFrame('snap',true)].join(',')!=='0,1,2,3')throw Error('painted fauna frames');");
  ...
  await check("newRun();mode='play';level=6;worldStage=worldFrom=1;worldNotice=0;hint=0;ship.inv=0;wallFauna=[];let a=spawnWallFauna('spitter',-1);a.y=118;a.ct=20;a=spawnWallFauna('snap',1);a.y=205;a.ct=18;render();");await shot('phase3-salvage-fauna');
  await check("const painted=IMG.wall_fauna;IMG.wall_fauna=null;render();IMG.wall_fauna=painted;");await shot('phase3-salvage-fauna-fallback');
  await check("worldStage=worldFrom=2;wallFauna=[];let a=spawnWallFauna('spitter',1);a.y=140;a.ct=22;render();");await shot('phase3-wilds-fauna');
  ...
  await check("worldStage=worldFrom=4;level=21;routeSegment=null;wallFauna=[];let a=spawnWallFauna('snap',-1);a.y=170;a.ct=12;render();");await shot('phase3-brood-fauna');
  if(errors.length)throw Error(errors.join('\n'));
  console.log('PASS painted fauna/fallback, cache/tells/fire/cadence/damage/pause/lifecycle, split telegraph/collision/mature clearance/rejoin and reachable rewards');ws.close();finish(0);
