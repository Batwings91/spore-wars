...
(async()=>{
  ...
  await check("for(let s=1;s<=4;s++){const c=faunaDecor(s);if(c.width!==X(PW)||c.height!==FAUNA_DECOR_H||faunaDecor(s)!==c)throw Error('decor cache '+s);}if(FAUNA_DECOR.filter(Boolean).length!==1||!FAUNA_DECOR[4])throw Error('decor eviction');if(WALL_FAUNA_LIMIT!==3)throw Error('fauna cap');");
