...
(async()=>{
  ...
  for(let gun=0;gun<6;gun++){
    await check("wpn="+gun+";save.engine=2;shield=2;podOpen=wpn>=1?24:0;rocketFlash=0;render()");await shot('ship-tier-'+gun);
  ...
  await check("newRun();mode='play';orbActive=false;enemies=[];boss=null;const target={x:ship.x,y:100,hp:100,anchor:0};ground=[target];for(let gun=1;gun<=5;gun++){wpn=gun;resetRockets();shots=[];const launches=[];for(let tick=1;tick<=800;tick++){updateRockets();if(shots.length){if(shots[0].target!==target||shots[0].dmg!==1)throw Error('ground missile');launches.push(tick);shots=[];}}if(launches[0]!==23+POD_RELOAD[gun])throw Error('first launch '+gun);for(let i=1;i<launches.length;i++)if(launches[i]-launches[i-1]!==POD_RELOAD[gun])throw Error('cadence');rocketT=0;shots=Array.from({length:POD_LIMIT[gun]},()=>({rocket:true,x:ship.x,y:200,vx:0,vy:-2,life:180,target}));updateRockets();if(shots.length!==POD_LIMIT[gun])throw Error('pod cap');}");
  await check("newRun();mode='play';orbActive=false;bossWarn=0;level=1;groundTimer=0;updateGround();if(groundTimer!==150)throw Error('ground interval');level=4;groundTimer=0;updateGround();if(groundTimer!==90)throw Error('late ground interval');const e=ground[0];e.y=120;e.anchor=worldScroll-X(120);e.hp=0;destroyGround(e);destroyGround(e);if(groundWrecks.length!==1)throw Error('duplicate wreck');for(let i=0;i<150;i++)updateGround();if(groundWrecks[0].age!==150)throw Error('wreck age');wpn=1;podOpen=24;ship.inv=0;hint=0;worldNotice=0;groundWrecks[0].x=320;groundWrecks[0].anchor=worldScroll-X(180);render()");await shot('side-missiles-wreck-smoke');
  ...
  console.log('PASS ground missile kill, side cadence/caps/wrecks, six tiers, Siege fire, orb pickup/shop/save/reload, cadence/cap/targeting, death/Continue/pause and keyboard/touch shop');console.log(errors);ws.close();finish(errors.length?1:0);
