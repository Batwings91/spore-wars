...
(async()=>{
  ...
  await check("if(save.rockets!==0)throw Error('fresh save ordnance');newRun();mode='play';ship.inv=0;hint=0;level=1;worldNotice=0;if(!IMG.player_hull)throw Error('hull missing');");
  ...
  await check("save.weapon=2;save.engine=2;save.shield=2;save.rockets=1;save.orb=1;save.cores=300;mode='shop';shopSel=0;render()");await shot('ship-workshop');
  ...
  await check("shopFromSector=false;const old=drawShipAssembly;let got;drawShipAssembly=(x,y,loadout)=>{got=loadout;};drawEquipmentPreview('primary',4,0,0);drawShipAssembly=old;if(got.weapon!==4||got.engine!==2||got.shield!==2||got.rockets!==1||got.orb!==1||got.sideLaser!==0)throw Error('preview lost equipment');");
  ...
  await check("const before=savedLoadout(),candidate=previewLoadout('ordnance',0);for(const k of Object.keys(before))if(k!=='rockets'&&candidate[k]!==before[k])throw Error('candidate changed '+k);if(candidate.rockets!==0)throw Error('candidate ordnance');const laser=previewLoadout('sideWeapon',1);if(laser.sideLaser!==0)throw Error('laser equipped');");
  await check("for(let i=0;i<GUN.length;i++)if(JSON.stringify(GUN_PORTS[i])!==JSON.stringify(GUN[i].shots.map(s=>s.slice(0,2))))throw Error('gun mount drift');if(EQUIPMENT.ordnance.tiers[1].reload!==180||SHOP[3]!==EQUIPMENT.ordnance)throw Error('equipment catalogue drift');");
  ...
  await check("mode='shop';shopSel=5;save.cores=500;const before=JSON.stringify(save);buy();if(JSON.stringify(save)!==before)throw Error('locked laser purchase');");
  ...
  await go(BASE);await evalJs('requestAnimationFrame=()=>0');await sleep(100);await check("newRun();if(save.cores!==37||save.best!==999||save.weapon!==2||save.shield!==1||save.engine!==3||save.orb!==1||save.rockets!==1||!orbActive)throw Error('legacy save migration');const stored=JSON.parse(localStorage.getItem(KEY));if(stored.rockets!==1)throw Error('migration not persisted');");
