...
(async()=>{
  ...
  await check("if(rollCoreValue(0.1)!==1||rollCoreValue(0.91)!==2||rollCoreValue(0.981)!==5||rollCoreValue(0.999)!==10)throw Error('core tiers');newRun();mode='play';cores=0;drops=[{k:'core',value:10,x:ship.x,y:ship.y}];update();if(cores!==10||!floats.some(f=>f.txt==='+10 CORES'&&f.y<LH))throw Error('core value/visible text');");
  ...
  await check("shopInstalled=null;mode='shop';shopSel=2;save.engine=2;save.cores=0;sell();if(save.engine!==1||save.cores!==60)throw Error('engine buyback');sell();if(save.engine!==1||save.cores!==60)throw Error('repeat sale');shopInstalled=null;shopFromSector=true;shopSel=6;lives=3;sell();if(lives!==2||save.cores!==135)throw Error('life sale');sell();if(lives!==2||save.cores!==135)throw Error('repeat life sale');shopFromSector=false;");
  ...
  await check("newRun();mode='play';orbActive=false;enemies=[];boss=null;const target={x:ship.x,y:100,hp:100,anchor:0},profile=EQUIPMENT.ordnance.tiers[1];ground=[target];save.rockets=0;wpn=5;resetRockets();for(let i=0;i<500;i++)updateRockets();if(podOpen!==0||shots.length)throw Error('gun unlocked rockets');save.rockets=1;wpn=0;resetRockets();shots=[];const launches=[];for(let tick=1;tick<=600;tick++){updateRockets();if(shots.length){const s=shots[0],mount=SHIP_MOUNTS.ordnance[launches.length%2];if(s.target!==target||s.dmg!==profile.damage||!s.lethal)throw Error('ground missile');if(s.x!==ship.x+mount[0]||s.y!==ship.y+mount[1])throw Error('launch origin');launches.push(tick);shots=[];}}if(launches[0]!==23+profile.reload)throw Error('first launch');for(let i=1;i<launches.length;i++)if(launches[i]-launches[i-1]!==profile.reload)throw Error('cadence');rocketT=0;shots=Array.from({length:profile.limit},()=>({rocket:true,x:ship.x,y:200,vx:0,vy:-2,life:180,target}));updateRockets();if(shots.length!==profile.limit)throw Error('pod cap');if(shotDamage({lethal:true,dmg:1},{hp:47})!==47||shotDamage({rocket:true,orb:true,dmg:1},{hp:47})!==1)throw Error('lethal pod damage contract');");
  ...
  await check("newRun();mode='play';orbActive=false;wpn=0;fireT=999;waveT=999;level=1;formationGroup=AUTHORED_WAVES[0].length;const e={x:ship.x,y:ship.y-70,anchor:worldScroll-X(ship.y-70),hp:7,stage:0,variant:0,ct:200,aim:0,flash:0};ground=[e];enemies=[];shots=[{x:e.x,y:e.y,vx:0,vy:-1,rocket:true,lethal:true,g:0,dmg:1,target:e,life:180,retargeted:false}];update();if(ground.includes(e)||groundWrecks.length!==1||!floats.some(f=>f.txt==='-7'))throw Error('lethal missile ground kill/damage');");
  console.log('PASS equipment catalogue/mount parity, full-loadout preview, v3 migration, independent lethal rockets, six tiers, orb, ground missile kill, pause and grid keyboard/touch shop');console.log(errors);ws.close();finish(errors.length?1:0);
