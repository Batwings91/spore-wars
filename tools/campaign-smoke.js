...
(async()=>{
  ...
  for(let stage=0;stage<5;stage++){
    ...
    await check("cores=10;bankedCores=0;save.cores=0;"+(stage===4?"score=500;":"")+"bossDying=1;updateBoss();if(!sectorPending)throw Error('boss reward');completeSector();if(save.cores!==10||mode!=="+JSON.stringify(stage===4?'victory':'sector')+")throw Error('completion');completeSector();if(save.cores!==10)throw Error('double bank');t=80;render()");
    if(stage===0)await check("const before=save.cores;return requestDoubleCores().then(()=>{if(!sectorDoubled||save.cores!==before+sectorBanked)throw Error('double cores reward');return true})");
  ...
  await check("if(!highScoreEntry)throw Error('victory high-score entry');highScoreEntry.letters=['T','S','T'];submitHighScore();render()");await shot('campaign-victory');
