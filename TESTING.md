...
The lower panel must show the whole ship with only the candidate slot overridden. From the title, other slots match the saved launch fit; from a checkpoint, they match the actual in-run weapon, remaining shield, side laser and run-owned Orb. Check primary, shield, engine, rocket, side-laser and Orb candidates while other modules remain visible. Preview rendering must not change save/current weapon/shields/rocket or laser timers/pod side. Verify side-laser affordability, purchase, duplicate/max handling and persistence plus Back/Continue, `player_hull.png` fallback, 640×360 readability and a smaller landscape viewport.
...
Check title Workshop BACK versus checkpoint CONTINUE (returns to results, never banks again). Level 2 must reveal violet scenery immediately, remain violet throughout waves 6–10, and use the same colour treatment if world_salvage is blocked; ship/projectile colours stay unchanged. Inspect Lurker body, smooth tentacles and identical tip hit positions with and without its WebP. Wave 11 introduces Spore Skimmers alongside lurkers/crawlers; check all four authored waves and procedural wave 16. Verify 4HP, score 60, projectile/rocket/bomb hits, ship collision, offscreen removal, drops, pause and procedural sprite fallback. Compare Brood Mother warning and spawned escort sounds: no FM notes, no clipping or stuck sound, cached replay, mute/blur still silence output.
...
Verify 45% core rolls on ordinary eligible air kills; unchanged 2% bombs,6%/2% shields and 4% conditional repairs; guaranteed W still takes precedence. Ground gun/rocket/bomb kills each leave exactly one wreck and roll one 50% core, including simultaneous hits. Collect/bank new cores at death/checkpoint/quit; Continue never re-banks old earnings. Check wrecks scroll at scenery speed, remain harmless, persist during boss entrance, cap 20 and clear on lifecycle resets. Ground density builds through each stage at 210/180/150/120 ticks with 5/5/5/6 caps and no boss spawns.
