...
function stepLogic(){if(!paused)ship.hullDisplay+=Math.max(-0.08,Math.min(0.08,ship.hull-ship.hullDisplay));if(exitWait>0)exitWait--;blink++;if(mode!=='play'&&!paused){t++;if(mode!=='sector')scroll=(scroll+0.9)%TH;}
  ...
  else if(mode==='shop'){if(tapped){if(tapSrc==='key'){if(shopSel===SHOP.length)leaveShop();else buy();}else{const i=shopHitTest(ptr.x,ptr.y);if(i>=0){if(shopSel!==i)shopInstalled=null;shopSel=i;shopItem=i;}else if(ptr.y>=298&&ptr.y<=330){if(ptr.x>=510&&ptr.x<=614)leaveShop();else if(ptr.x>=292&&ptr.x<=500&&shopSel<SHOP.length)buy();}}}}
