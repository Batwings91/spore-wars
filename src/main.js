...
newRun();
ads.configure({onAdState:on=>{adPlaying=on;tapped=false;ptr.down=false;for(const k in keys)keys[k]=false;},onMuteChange:SFX.setExternalMute});
let lastT=0,acc=0;
...
function stepLogic(){const gameplay=(mode==='play'||mode==='travel')&&(!paused||focusPaused)&&!adPlaying;ads.setGameplay(gameplay);if(adPlaying){SFX.music(false);tapped=false;return;}if(rewardNoticeT>0){rewardNoticeT--;if(rewardNoticeT===0)rewardNotice='';}if(!paused)ship.hullDisplay+=Math.max(-0.08,Math.min(0.08,ship.hull-ship.hullDisplay));if(exitWait>0)exitWait--;blink++;if(mode!=='play'&&!paused){t++;if(mode!=='sector')scroll=(scroll+0.9)%TH;}
