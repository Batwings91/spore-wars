...
function updateBoss(){
  ...
  for(const s of shots){if(s.y<-50)continue;let hit=false;
    for(const tu of b.turrets){if(tu.hp>0&&Math.abs(s.x-(b.x+tu.dx))<14&&Math.abs(s.y-(b.y+tu.dy))<14){const damage=shotDamage(s,tu);tu.hp-=damage;hit=true;addHitImpact(s.x,s.y,b.kind>=2||b.mother);addDamageFloat(s.x,s.y,damage,b.kind>=2||b.mother);if(tu.hp<=0){boom(b.x+tu.dx,b.y+tu.dy,false);awardKill(150,b.x+tu.dx,b.y+tu.dy);}break;}}
    if(!hit&&(b.mother?((Math.abs(s.x-b.x)<32&&Math.abs(s.y-b.y)<142)||(Math.abs(s.x-b.x)<96&&Math.abs(s.y-b.y)<62)):(Math.abs(s.x-b.x)<(b.mech?54:80)&&Math.abs(s.y-b.y)<(b.mech?36:60)))){const damage=shotDamage(s,b);b.hp-=damage;b.flash=3;hit=true;addHitImpact(s.x,s.y,b.kind>=2||b.mother);addDamageFloat(s.x,s.y,damage,b.kind>=2||b.mother);SFX.hit();}
    if(hit)s.y=-99;}
