...
function spawnBoss(){bossCount++;const mech=level%15===10,mother=level%15===0;
  ...
  const hpMax=mother?Math.round(300*(1+level*0.02)):mech?Math.round(260*(1+level*0.02)):Math.round((140+nth*60)*(1+level*0.02)*(nth===1?0.9:1));
...
function updateBoss(){
  ...
  if(bossDying>0){bossDying--;
    ...
    if(bossDying===0){booms.push({x:b.x,y:b.y,f:0,life:40,kind:'exp_boss',sc:1.4});booms.push({x:b.x,y:b.y+20,f:0,life:60,kind:'smoke',sc:2});flash=14;shake=26;slow=30;
      score+=2000+bossCount*500;addFloat(b.x,b.y-40,'+'+(2000+bossCount*500),C.G,true);pickupEvent(b.mother?'BROOD MOTHER DEFEATED':b.mech?'MECH DESTROYED':'BATTLESHIP DESTROYED',C.G);
...
function drawBoss(){
  ...
  // health bar
  const bw=PW-40;glassPanel(PX+20,6,bw,20,'#b88364');const f=Math.max(0,Math.min(1,b.hp/b.hpMax));txt(b.mother?'BROOD MOTHER':b.mech?'ASSAULT MECH':'BATTLESHIP',PX+28,9,'#e6c6b0',9);ctx.fillStyle='#293642';ctx.fillRect(X(PX+145),X(12),X(bw-135),X(5));ctx.fillStyle=b.phase===2?'#ed7f86':'#eeb275';ctx.fillRect(X(PX+145),X(12),X((bw-135)*f),X(5));
}
