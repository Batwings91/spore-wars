...
let bankedCores=0,sectorPending=false,sectorBanked=0,sectorDoubled=false,shopFromSector=false;
const MAX_HULL=3;
...
function newRun(loop=0){startScoreRun();campaignLoop=loop;const startWave=loop>0?0:Math.min(24,STARTWAVE),starting=savedLoadout();resetGround();resetWorldEncounters();formationGroup=startWave?(AUTHORED_WAVES[startWave-1]?.length||0):0;sectorPending=false;sectorBanked=0;sectorDoubled=false;shopFromSector=false;shopInstalled=null;resetRockets();resetSideLasers();resetWorld(startWave+1);chain=0;chainT=0;bankedCores=0;ship={x:PX+PW/2,y:LH-60,inv:60,hull:MAX_HULL,hullDisplay:MAX_HULL};shots=[];eshots=[];enemies=[];drops=[];booms=[];
...
const AUTHORED_WAVES=[
  [[0,2,0.5],[0,2,0.28],[0,2,0.72],[0,2,0.4],[0,2,0.65],[0,2,0.3],[0,2,0.7]],
  [[0,2,0.25],[0,2,0.75],[0,2,0.5],[0,2,0.3],[0,2,0.7],[0,2,0.4],[0,2,0.6]],
  [[0,2,0.5],[1,2,0.3],[1,2,0.7],[0,2,0.25],[1,2,0.6],[1,2,0.4],[0,2,0.7]],
  [[2,1,0.5],[1,2,0.5],[2,2,0.5],[1,2,0.3],[2,1,0.7],[1,2,0.65],[2,1,0.35]],
  null,
  // Infected Salvage: introduce lurkers singly; crawlers first appear in wave 9.
  [[0,2,0.5],[3,1,0.5],[0,2,0.3],[3,1,0.7],[0,2,0.7],[3,1,0.3]],
  [[3,1,0.3],[1,2,0.6],[2,1,0.7],[3,1,0.5],[1,2,0.3],[2,1,0.6]],
  [[2,1,0.3],[3,1,0.7],[0,2,0.5],[3,1,0.35],[0,2,0.25],[3,1,0.6]],
  [[4,1,0.06],[0,2,0.7],[4,1,0.94],[3,1,0.5],[4,1,0.06],[0,2,0.4]],
  null,
...
function spawnFormationGroup(){
  ...
  const first=enemies.length;formationGroup++;
...
function spawnWave(){if(level>=CAMPAIGN_WAVES)return;level++;if(level>1)SFX.wave();if(AUTHORED_WAVES[level-1]){formationGroup=0;spawnFormationGroup();return;}waveT=90;const n=Math.min(8,3+Math.floor(level/2));
